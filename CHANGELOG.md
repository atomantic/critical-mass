...
### Fixed
- **Dry-run immediate state save no longer drops a debounce-queued fund** - `saveState`'s non-debounced branch called `pendingStates.clear()` and wrote only the current fund, discarding any fund queued by an earlier debounced call and leaving the scheduled timer to later flush an empty map. The immediate branch now cancels the pending timer and flushes every queued fund before writing, so a multi-fund setup can't silently lose a snapshot when one fund saves immediately while another is queued (#159)
