...
### Added
- **UpDown paper-book P&L monitor** — `scripts/updown-pnl-snapshot.js` snapshots 0.01-BTC-per-contract mark-to-market P&L (`PROFITABLE` / `UNDERWATER` / `FLAT` / `NO_FILLS`) and can `--hydrate-state` from signal history + scorecard prices so a restart does not look like a flat book. New history rows store `price` so later replays do not need the journal.
