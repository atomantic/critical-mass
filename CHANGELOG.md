...
### Changed
- **[issue-366] Memoize P&L enrichment and remove redundant sort in TransactionsRegime** — Wrap computeFillsWithPnL with useMemo, eliminate unused sortedFills sort (1.79ms saved), and consolidate 8 separate summary stat filter/reduce passes into one memoized computation loop, reducing render-time overhead from ~20ms to near-zero on 25K-fill datasets.
