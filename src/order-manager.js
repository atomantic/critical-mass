 ...
 */
const waitForBuyFill = async (orderId, maxAttempts = 10, delayMs = 1000) => {
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    const order = await api.getOrder(orderId);

    if (order.status === 'FILLED' || order.completionPercentage >= 100) {
      // Get detailed fill info with fees/rebates
      const fillSummary = await api.getOrderFillSummary(orderId);
 ...
 * @param {number} usdcAmount - Amount to spend in USDC
 * @returns {Promise<Object>} Buy result with fill details
 */
const executeDailyBuy = async (config, usdcAmount) => {
  log('INFO', `Placing market buy for ${usdcAmount} USDC of ${config.productId}`);

  // Place the market buy
  const buyResult = await api.placeMarketBuy(config.productId, usdcAmount);
  ...
  // Wait for fill
  const fillDetails = await waitForBuyFill(buyResult.orderId);

  log('INFO', `Buy filled: ${fillDetails.btcAmount.toFixed(8)} BTC at ${fillDetails.price.toFixed(2)} USDC`);
  log('INFO', `Fees: ${fillDetails.fees.toFixed(4)} USDC, Rebates: ${fillDetails.rebates.toFixed(4)} USDC, Net: ${fillDetails.netFees.toFixed(4)} USDC`);
 ...
 */
const placeSellOrder = async (config, buyDetails) => {
  ...
  log('INFO', `Placing post-only sell for ${sellQuantity} BTC at ${sellPrice} USDC`);

  const sellResult = await api.placeLimitSell(config.productId, sellQuantity, sellPrice);
 ...
 */
const checkFilledOrders = async (pendingOrders) => {
  const filledOrders = [];

  for (const pendingOrder of pendingOrders) {
    const orderStatus = await api.getOrder(pendingOrder.orderId);

    if (orderStatus.status === 'FILLED') {
      // Get detailed fill info with fees/rebates
      const fillSummary = await api.getOrderFillSummary(pendingOrder.orderId);
 ...
 * @returns {Promise<Object>} Sell order result
 */
const placeSellOrderWithRetry = async (config, buyDetails, maxRetries = 3) => {
  let lastError;
