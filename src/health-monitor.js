...
// ============================================================================
// Adapter instrumentation for health monitoring
// ============================================================================
...
const isAuthDeniedError = (err) => {
  ...
  return /unauthorized|not\s*authenticated|ip[_\s-]*illegal|ip\s*address\s*not\s*(?:whitelist|allow)|not\s*(?:whitelist|allow)(?:ed|listed)|invalid\s*api[_\s-]*key/i.test(msg);
};
...
module.exports = {
  ...
  isRateLimitError,
  isAuthDeniedError,
  INSTRUMENTED_REST_METHODS,
  AUTHENTICATED_REST_METHODS,
};
