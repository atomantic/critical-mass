...
- "WS fill for untracked order — likely orphan" warning was firing on every polling-backstop fill, not just real orphans. The polling paths (`scheduleStaleOrderTimeout`, `refreshStaleOrders`, `checkPendingOrderFills`, `handleCancelledOrder` partial-fill) delete from `pendingOrders` BEFORE dispatching the fill through the engine; the engine's `handleOrderFill` chain runs `orderExecutor.handleOrderFill(orderId)` at the end and would find no entry, logging a 🚨 even though the fill was handled correctly. New `recentlySettled` TTL Map (5min, 256-entry cap) discriminates real orphans from benign double-cleanup. Warning now only fires when neither `pendingOrders` nor `recentlySettled` knows the orderId.
